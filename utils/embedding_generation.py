...
def generate_umap(patch_path, experiment_name, embeddings, filenames, sample_name):
    # backbone.eval()
    #
    ...
    for filename in filenames:
        ...
        x = int(re.search('x(.*).jpg', filename).group(1))
        pos_y.append(y)
        pos_x.append(x)

    df = pd.DataFrame(np.hstack([np.array([components[:, 0], components[:, 1], pos_x, pos_y]).transpose(), hex_feat]),
                      columns=['UMAP1', 'UMAP2', 'x', 'y', 'color'])
