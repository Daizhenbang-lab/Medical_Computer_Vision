...
def plot_patches_in_image_space(csv_path, image_size, patch_size,sample_name, save_path):
    ...
    for _, row in coordinates_df.iterrows():
        if row['sample_name'] == sample_name[ :-4]:
            top_left = (int(row['x'] - patch_size / 2), int(row['y'] - patch_size / 2))
            bottom_right = (int(row['x'] + patch_size / 2), int(row['y'] + patch_size / 2))
...
parser.add_argument('--sample_slide', type=str, default='full_sample/sample',
                        help='directory of saving original samples')
parser.add_argument('--umap_file', type=str, default='test_sample_56_overlap0.5/sample/UMAP_test_neighbor2.csv',
                        help='the csv file of UMAP')
parser.add_argument('--save_path', type=str, default='patch_project_56',
